export default class GameScene extends Phaser.Scene {
  ...
  spawnEnemyShip() {
    this.enemyDestroying = false;
    const { width } = this.scale;
    this.enemyShipHits = 0;
    this.enemyShipHp = this.enemyHitsToDestroy;
    ...
    const startY = -this.enemyShip.displayHeight * 0.5;
    const hpToUse = this.enemyShipHp;
    const vx = 0;
  ...
  handleBulletHitEnemy(bullet, enemy) {
    // Kill the projectile immediately on a hit.
    bullet.disableBody(true, true);
    this.bullets.killAndHide(bullet);
    if (bullet.body) bullet.body.enable = false;
    if (!enemy.active) return;
    const currentHp = enemy.getData('hp') ?? this.enemyShipHp ?? this.enemyHitsToDestroy;
    const newHp = currentHp - 1;
    enemy.setData('hp', newHp);
    this.enemyShipHits += 1;
    if (newHp <= 0 || this.enemyShipHits >= this.enemyHitsToDestroy) {
      this.destroyEnemyShip(enemy, { snapshot: true, hp: currentHp, reduceCount: true, respawnDelayMs: 0 });
    }
  }
  ...
  destroyEnemyShip(enemy, options = {}) {
    const { snapshot = false, hp, reduceCount = true, respawnDelayMs = 0 } = options;
    if (this.enemyDestroying) return;
    this.enemyDestroying = true;
    if (snapshot) {
    ...
    this.enemyOverlap?.destroy();
    this.enemyOverlap = null;
    this.enemyShipHp = null;
    if (reduceCount) {
      this.remainingEnemies = Math.max(0, this.remainingEnemies - 1);
      this.updateEnemyCountLabel();
    }
    this.enemyShip = null;
    this.time.delayedCall(respawnDelayMs, () => this.spawnEnemyShip());
  }
  ...
  handleEnemyOutOfBounds() {
    ...
    if (outOfHorizontal || outBelow) this.destroyEnemyShip(this.enemyShip, { snapshot: false, reduceCount: false });
  }
